...
export default function DashboardPage() {
  ...
  const statCards = [
    ...
    { label: t('dashboard.todaysRevenue', "Today's Revenue"), value: `${dashStats?.todayRevenue ?? 0} NIS`, icon: <Coins className="h-6 w-6" /> },
  ];

  const statusLabels: Record<string, string> = {
    received: t('orders.status.received', 'Received'),
    in_progress: t('orders.status.inProgress', 'In Progress'),
