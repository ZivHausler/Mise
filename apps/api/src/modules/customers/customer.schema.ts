import { z } from 'zod';
import { dateStringSchema } from '@mise/shared';

export const createCustomerSchema = z.object({
  name: z.string().min(1, 'NAME_REQUIRED').max(200),
  phone: z.string().min(1, 'PHONE_REQUIRED').max(50),
  email: z.union([z.string().email('INVALID_EMAIL_ADDRESS').max(255), z.literal('')]).optional(),
  address: z.string().max(500).optional(),
  notes: z.string().max(2000).optional(),
  preferences: z.object({
    allergies: z.array(z.string().max(100)).max(50).optional(),
    favorites: z.array(z.string().max(100)).max(50).optional(),
  }).optional(),
  birthday: dateStringSchema
    .refine((val) => !isNaN(new Date(val).getTime()), 'INVALID_DATE')
    .nullable()
    .optional(),
  loyaltyEnabled: z.boolean().optional(),
  loyaltyTier: z.enum(['bronze', 'silver', 'gold']).optional(),
});

export const updateCustomerSchema = createCustomerSchema.partial();
